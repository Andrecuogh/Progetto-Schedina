...
class LeagueSeason:
    ...
    def export_csv(self, path):
        for i in range(self.days):
            self.leagues[i].to_csv(f'{path}/scores/{self.year}/{i}.csv')
            self.rankings[i].to_csv(f'{path}/rankings/{self.year}/{i}.csv')
            self.results[i].to_csv(f'{path}/results/{self.year}/{i}.csv')
            self.gsgr[i].to_csv(f'{path}/scored_received/{self.year}/{i}.csv')