...
def create_matchday_df(LS, g):
    ...
    for incontro in tables['partite'].index:
        sfida = tables['partite'].iloc[incontro,0] + '-' + tables['partite'].iloc[incontro,1]
        df.iloc[incontro, 0] = sfida

        if not tables['prediction']:
            df.loc[incontro, 'yGf'] = tables['partite'].iloc[incontro, 2].split('-')[0]
            df.loc[incontro, 'yGs'] = tables['partite'].iloc[incontro, 2].split('-')[1]
...
def create_year_df(LS):
    ...
    year_df.to_csv(f'/content/drive/MyDrive/Schedina/data/dataframes/{LS.year}.csv')
